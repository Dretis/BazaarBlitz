...
public class SettingsManager : MonoBehaviour
{
    ...
    {
        SaveSettings(); // auto-save on quit
    }
    ...
    {
        gameSettings.musicVolume = Mathf.Clamp01(newVolume);
        Debug.Log($"Music Volume set to: {gameSettings.musicVolume}");
    }

    // Updates the SFX volume in GameSettings, clamped 0-1
    public void SetEffectsVolume(float newVolume)
    {
        gameSettings.effectsVolume = Mathf.Clamp01(newVolume);
        Debug.Log($"SFX Volume set to: {gameSettings.effectsVolume}");
    }
    /*
